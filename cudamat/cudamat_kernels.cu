...
__global__ void kAssignScalar(float* dest, float alpha, unsigned long long len) {
  const unsigned int idx = blockIdx.x * blockDim.x + threadIdx.x;
  const unsigned int numThreads = blockDim.x * gridDim.x;
  for (unsigned long long i = idx; i < len; i += numThreads) {
    dest[i] = alpha;
  }
}
...
__global__ void kSumAll(float* mat, float* target, unsigned long long len,
    unsigned long long len_per_block, unsigned int left_over) {
  extern __shared__ float sum_vals[];
  int block_id = blockIdx.x;
  mat += block_id * len_per_block + (block_id < left_over ? block_id : left_over);
  unsigned long long l = len_per_block + (block_id < left_over ? 1 : 0);
  
  __syncthreads();
  float cur_sum = 0;
  for (unsigned long long i = threadIdx.x; i < l; i += blockDim.x) {
    cur_sum += mat[i];
  }
  __syncthreads();
  sum_vals[threadIdx.x] = cur_sum;
...
__global__ void kLSTMFprop2Init(float *gates, float* cell, float* output, float* w, int num_lstms, int num_cases) {
  ...
  if (lstm_id < num_lstms && case_id < num_cases) {
    gates    += case_id * num_lstms * 4 + lstm_id;
    cell     += case_id * num_lstms + lstm_id;
    output   += case_id * num_lstms + lstm_id;
    float i    = gates[0],
          f    = gates[num_lstms],
          a    = gates[num_lstms * 2],
          o    = gates[num_lstms * 3];
...
__global__ void kLSTMFprop2(float *gates, float* cell_prev, float* cell, float* output, float* w, int num_lstms, int num_cases) {
  ...
  if (lstm_id < num_lstms && case_id < num_cases) {
    gates     += case_id * num_lstms * 4 + lstm_id;
    cell_prev += case_id * num_lstms + lstm_id;
    cell      += case_id * num_lstms + lstm_id;
    output    += case_id * num_lstms + lstm_id;
    w         += lstm_id;
    float i    = gates[0],
...
__global__ void kLSTMBprop2Init(float *gates, float* gates_deriv, float* cell, float* cell_deriv,
  ...
  if (lstm_id < num_lstms && case_id < num_cases) {
    gates         += case_id * num_lstms * 4 + lstm_id;
    gates_deriv   += case_id * num_lstms * 4 + lstm_id;
    cell          += case_id * num_lstms + lstm_id;
    cell_deriv    += case_id * num_lstms + lstm_id;
    output_deriv  += case_id * num_lstms + lstm_id;
    ...
    __syncthreads();
    gates_deriv[0] = i_d;
    gates_deriv[num_lstms] = 0;
    gates_deriv[2 * num_lstms] = a_d;
    gates_deriv[3 * num_lstms] = o_d;
    cell_deriv[0] = c_d;
  }
}
__global__ void kLSTMBprop2(float *gates, float* gates_deriv, float* cell_prev, float* cell_prev_deriv,
  ...
  if (lstm_id < num_lstms && case_id < num_cases) {
    ...
    cell_deriv      += case_id * num_lstms + lstm_id;
    cell_prev       += case_id * num_lstms + lstm_id;
    cell_prev_deriv += case_id * num_lstms + lstm_id;
    output_deriv    += case_id * num_lstms + lstm_id;
    w               += lstm_id;
    ...
    __syncthreads();
    gates_deriv[0] = i_d;
    gates_deriv[num_lstms] = f_d;
    gates_deriv[2 * num_lstms] = a_d;
    gates_deriv[3 * num_lstms] = o_d;
    cell_deriv[0] = c_d;
    cell_prev_deriv[0] = c_p_d;
  }
}

__global__ void kLSTMOutp2Init(const float* gates_deriv, const float* cell, float* dw, int num_lstms, int num_cases) {
  extern __shared__ float sum_vals[];
  const int lstm_id = gridDim.x * blockIdx.y + blockIdx.x;
  if (lstm_id < num_lstms) {
    cell += lstm_id;
    dw += lstm_id;
    const float* o_d = gates_deriv + 3 * num_lstms + lstm_id;
    float dwo = 0.0f;
    for (unsigned int i = threadIdx.x; i < num_cases; i += blockDim.x) {
      dwo += cell[i * num_lstms] * o_d[i * 4 * num_lstms];
    }
    sum_vals[threadIdx.x] = dwo;
    reduceToSumLocal32(sum_vals, threadIdx.x);
    __syncthreads();
    if (threadIdx.x == 0) {
      dw[2 * num_lstms] += sum_vals[0];
    }
  }
}

__global__ void kLSTMOutp2(const float* gates_deriv, const float* prev_cell,
                           const float* cell, float* dw, int num_lstms, int num_cases) {
  extern __shared__ float sum_vals[];
  const int lstm_id = gridDim.x * blockIdx.y + blockIdx.x;
  if (lstm_id < num_lstms) {
    cell += lstm_id;
    prev_cell += lstm_id;
    const float* i_d = gates_deriv + lstm_id;
    const float* f_d = gates_deriv + num_lstms + lstm_id;
    const float* o_d = gates_deriv + 3 * num_lstms + lstm_id;
    dw += lstm_id;
    float dwo = 0.0f, dwi = 0.0f, dwf = 0.0f;
    for (unsigned int i = threadIdx.x; i < num_cases; i += blockDim.x) {
      long ind = i * num_lstms;
      long d_ind = 4 * ind;
      float pcell = prev_cell[ind];
      dwi += pcell * i_d[d_ind];
      dwf += pcell * f_d[d_ind];
      dwo += cell[ind] * o_d[d_ind];
    }
    sum_vals[threadIdx.x] = dwi; reduceToSumLocal32(sum_vals, threadIdx.x); __syncthreads();
    if (threadIdx.x == 0) dw[0] += sum_vals[0];
    sum_vals[threadIdx.x] = dwf; reduceToSumLocal32(sum_vals, threadIdx.x); __syncthreads();
    if (threadIdx.x == 0) dw[num_lstms] += sum_vals[0];
    sum_vals[threadIdx.x] = dwo; reduceToSumLocal32(sum_vals, threadIdx.x); __syncthreads();
    if (threadIdx.x == 0) dw[2 * num_lstms] += sum_vals[0];
  }
}
...
__global__ void kLSTMFpropSpatialInit(float *gates, float* cell, float* output, float* w, int num_lstms, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int lstm_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (lstm_id < num_lstms && case_id < num_cases) {
    gates    += case_id * num_lstms * 5 + lstm_id;
    cell     += case_id * num_lstms + lstm_id;
    output   += case_id * num_lstms + lstm_id;
    float i    = gates[0],
          fr   = gates[num_lstms],
          fc   = gates[num_lstms * 2],
          a    = gates[num_lstms * 3],
          o    = gates[num_lstms * 4];
    float w_co = w[lstm_id + 4 * num_lstms];
    i = sigmoid(i);
    fr = sigmoid(fr);
    fc = sigmoid(fc);
    a = tanh(a);
...
__global__ void kLSTMFpropSpatialRowInit(float *gates, float* cell_prev_row, float* cell, float* output, float* w, int num_lstms, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int lstm_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (lstm_id < num_lstms && case_id < num_cases) {
    gates     += case_id * num_lstms * 5 + lstm_id;
    cell_prev_row += case_id * num_lstms + lstm_id;
    cell      += case_id * num_lstms + lstm_id;
    output    += case_id * num_lstms + lstm_id;
    w         += lstm_id;
    float i    = gates[0],
...
__global__ void kLSTMFpropSpatialColInit(float *gates, float* cell_prev_col, float* cell, float* output, float* w, int num_lstms, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int lstm_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (lstm_id < num_lstms && case_id < num_cases) {
    gates     += case_id * num_lstms * 5 + lstm_id;
    cell_prev_col += case_id * num_lstms + lstm_id;
    cell      += case_id * num_lstms + lstm_id;
    output    += case_id * num_lstms + lstm_id;
    w         += lstm_id;
    float i    = gates[0],
          ...
          o    = gates[num_lstms * 4],
          c    = cell_prev_col[0];
    float w_cic = w[num_lstms],
          w_cfc = w[3 * num_lstms],
          w_co  = w[4 * num_lstms];

    i = sigmoid(i + c * w_cic);
    fr = sigmoid(fr);
    fc = sigmoid(fc + c * w_cfc);
    a = tanh(a);
    c = c * fc + i * a;
    o = sigmoid(o + c * w_co);
    float r = tanh(c) * o;
...
__global__ void kLSTMFpropSpatial(float *gates, float* cell_prev_row, float* cell_prev_col, float* cell, float* output, float* w, int num_lstms, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int lstm_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (lstm_id < num_lstms && case_id < num_cases) {
    gates     += case_id * num_lstms * 5 + lstm_id;
    ...
    i  = sigmoid(i + cr * w_cir + cc * w_cic);
    fr = sigmoid(fr + cr * w_cfr);
    fc = sigmoid(fc + cc * w_cfc);
    a  = tanh(a);
    c  = cr * fr + cc * fc + i * a;
    o  = sigmoid(o + c * w_co);
    float r = tanh(c) * o;
...
__global__ void kLSTMBpropSpatialInit(float *gates, float* gates_deriv, float* cell, float* cell_deriv,
    float* output_deriv, float* w, int num_lstms, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int lstm_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (lstm_id < num_lstms && case_id < num_cases) {
    gates         += case_id * num_lstms * 5 + lstm_id;
    gates_deriv   += case_id * num_lstms * 5 + lstm_id;
    cell          += case_id * num_lstms + lstm_id;
    cell_deriv    += case_id * num_lstms + lstm_id;
    output_deriv  += case_id * num_lstms + lstm_id;
    ...
    gates_deriv[0] = i_d;
    gates_deriv[num_lstms] = 0;
    gates_deriv[2 * num_lstms] = 0;
    gates_deriv[3 * num_lstms] = a_d;
    gates_deriv[4 * num_lstms] = o_d;
    cell_deriv[0] = c_d;
  }
}

__global__ void kLSTMBpropSpatialRowInit(float *gates, float* gates_deriv, float* cell_prev_row, float* cell_prev_row_deriv,
    float* cell, float* cell_deriv, float* output_deriv, float* w, int num_lstms, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int lstm_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (lstm_id < num_lstms && case_id < num_cases) {
    ...
    cell_deriv      += case_id * num_lstms + lstm_id;
    cell_prev_row       += case_id * num_lstms + lstm_id;
    cell_prev_row_deriv += case_id * num_lstms + lstm_id;
    output_deriv    += case_id * num_lstms + lstm_id;
    w               += lstm_id;
    ...
    float a_d = c_d * i * deriv_of_tanh(a);
    float i_d = c_d * a * deriv_of_sigmoid(i);
    float fr_d = c_d * cr * deriv_of_sigmoid(fr);
    cr_d += c_d * fr + i_d * w_cir + fr_d * w_cfr;

    __syncthreads();
    gates_deriv[0] = i_d;
    gates_deriv[num_lstms] = fr_d;
    gates_deriv[2 * num_lstms] = 0;
    gates_deriv[3 * num_lstms] = a_d;
    gates_deriv[4 * num_lstms] = o_d;
    cell_deriv[0] = c_d;
    cell_prev_row_deriv[0] = cr_d;
  }
}

__global__ void kLSTMBpropSpatialColInit(float *gates, float* gates_deriv, float* cell_prev_col, float* cell_prev_col_deriv,
    float* cell, float* cell_deriv, float* output_deriv, float* w, int num_lstms, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int lstm_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (lstm_id < num_lstms && case_id < num_cases) {
    ...
    cell_deriv      += case_id * num_lstms + lstm_id;
    cell_prev_col       += case_id * num_lstms + lstm_id;
    cell_prev_col_deriv += case_id * num_lstms + lstm_id;
    output_deriv    += case_id * num_lstms + lstm_id;
    w               += lstm_id;
    ...
    float a_d = c_d * i * deriv_of_tanh(a);
    float i_d = c_d * a * deriv_of_sigmoid(i);
    float fc_d = c_d * cc * deriv_of_sigmoid(fc);
    float cc_d = c_d * fc + i_d * w_cic + fc_d * w_cfc;

    __syncthreads();
    gates_deriv[0] = i_d;
    gates_deriv[num_lstms] = 0;
    gates_deriv[2 * num_lstms] = fc_d;
    gates_deriv[3 * num_lstms] = a_d;
    gates_deriv[4 * num_lstms] = o_d;
    cell_deriv[0] = c_d;
    cell_prev_col_deriv[0] = cc_d;
  }
}

__global__ void kLSTMBpropSpatial(float *gates, float* gates_deriv, float* cell_prev_row, float* cell_prev_row_deriv, float* cell_prev_col,
    float* cell_prev_col_deriv, float* cell, float* cell_deriv, float* output_deriv, float* w, int num_lstms, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int lstm_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (lstm_id < num_lstms && case_id < num_cases) {
    gates           += case_id * num_lstms * 5 + lstm_id;
...
__global__ void kLSTMOutpSpatialInit(const float* gates_deriv, const float* cell, float* dw, int num_lstms, int num_cases) {
  extern __shared__ float sum_vals[];
  const int lstm_id = gridDim.x * blockIdx.y + blockIdx.x;
  if (lstm_id < num_lstms) {
    cell += lstm_id;
    dw += lstm_id;
    const float* o_d = gates_deriv + 4 * num_lstms + lstm_id;
    float dwo = 0.0f;
    for (unsigned int i = threadIdx.x; i < num_cases; i += blockDim.x) {
      dwo += cell[i * num_lstms] * o_d[i * 5 * num_lstms];
    }
    sum_vals[threadIdx.x] = dwo;
    reduceToSumLocal32(sum_vals, threadIdx.x);
    __syncthreads();
    if (threadIdx.x == 0) {
      dw[4 * num_lstms] += sum_vals[0];
    }
  }
}

__global__ void kLSTMOutpSpatialRowInit(const float* gates_deriv, const float* prev_cell_row,
                           const float* cell, float* dw, int num_lstms, int num_cases) {
  extern __shared__ float sum_vals[];
  const int lstm_id = gridDim.x * blockIdx.y + blockIdx.x;
  if (lstm_id < num_lstms) {
    cell += lstm_id;
    prev_cell_row += lstm_id;
    const float* i_d = gates_deriv + lstm_id;
    const float* fr_d = gates_deriv + num_lstms + lstm_id;
    const float* o_d = gates_deriv + 4 * num_lstms + lstm_id;
    dw += lstm_id;
    float dwo = 0.0f, dwir = 0.0f, dwfr = 0.0f;
    for (unsigned int i = threadIdx.x; i < num_cases; i += blockDim.x) {
      long ind = i * num_lstms;
      long d_ind = 5 * ind;
      float pcell_row = prev_cell_row[ind];
      dwir += pcell_row * i_d[d_ind];
      dwfr += pcell_row * fr_d[d_ind];
      dwo += cell[ind] * o_d[d_ind];
    }
    sum_vals[threadIdx.x] = dwir; reduceToSumLocal32(sum_vals, threadIdx.x); __syncthreads();
    if (threadIdx.x == 0) dw[0] += sum_vals[0];
    sum_vals[threadIdx.x] = dwfr; reduceToSumLocal32(sum_vals, threadIdx.x); __syncthreads();
    if (threadIdx.x == 0) dw[2 * num_lstms] += sum_vals[0];
    sum_vals[threadIdx.x] = dwo; reduceToSumLocal32(sum_vals, threadIdx.x); __syncthreads();
    if (threadIdx.x == 0) dw[4 * num_lstms] += sum_vals[0];
  }
}

__global__ void kLSTMOutpSpatialColInit(const float* gates_deriv, const float* prev_cell_col,
                           const float* cell, float* dw, int num_lstms, int num_cases) {
  extern __shared__ float sum_vals[];
  const int lstm_id = gridDim.x * blockIdx.y + blockIdx.x;
  if (lstm_id < num_lstms) {
    cell += lstm_id;
    prev_cell_col += lstm_id;
    const float* i_d = gates_deriv + lstm_id;
    const float* fc_d = gates_deriv + 2 * num_lstms + lstm_id;
    const float* o_d = gates_deriv + 4 * num_lstms + lstm_id;
    dw += lstm_id;
    float dwo = 0.0f, dwic = 0.0f, dwfc = 0.0f;
    for (unsigned int i = threadIdx.x; i < num_cases; i += blockDim.x) {
      long ind = i * num_lstms;
      long d_ind = 5 * ind;
      float pcell_col = prev_cell_col[ind];
      dwic += pcell_col * i_d[d_ind];
      dwfc += pcell_col * fc_d[d_ind];
      dwo += cell[ind] * o_d[d_ind];
    }
    sum_vals[threadIdx.x] = dwic; reduceToSumLocal32(sum_vals, threadIdx.x); __syncthreads();
    if (threadIdx.x == 0) dw[num_lstms] += sum_vals[0];
    sum_vals[threadIdx.x] = dwfc; reduceToSumLocal32(sum_vals, threadIdx.x); __syncthreads();
    if (threadIdx.x == 0) dw[3 * num_lstms] += sum_vals[0];
    sum_vals[threadIdx.x] = dwo; reduceToSumLocal32(sum_vals, threadIdx.x); __syncthreads();
    if (threadIdx.x == 0) dw[4 * num_lstms] += sum_vals[0];
  }
}

__global__ void kLSTMOutpSpatial(const float* gates_deriv, const float* prev_cell_row, const float* prev_cell_col,
                           const float* cell, float* dw, int num_lstms, int num_cases) {
  extern __shared__ float sum_vals[];
  const int lstm_id = gridDim.x * blockIdx.y + blockIdx.x;
  if (lstm_id < num_lstms) {
    ...
    const float* o_d = gates_deriv + 4 * num_lstms + lstm_id;
    dw += lstm_id;
    float dwo = 0.0f, dwir = 0.0f, dwic = 0.0f, dwfr = 0.0f, dwfc = 0.0f;
    for (unsigned int i = threadIdx.x; i < num_cases; i += blockDim.x) {
      long ind = i * num_lstms;
      long d_ind = 5 * ind;
      float pcell_row = prev_cell_row[ind];
...
__global__ void kGetGlimpsesMatrixScan(float* F_x, float* F_y, int start_row, int start_col, int patch_side, int image_side, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int side_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (side_id < patch_side*image_side && case_id < num_cases) {
    F_x += case_id * patch_side * image_side + side_id;
    F_y += case_id * patch_side * image_side + side_id;

    int r = side_id / image_side;
    int c = side_id % image_side;

    int fy = 0;
    if (side_id == r*image_side+start_row+r) {
      fy = 1;
    }

    int fx = 0;
    if (side_id == r*image_side+start_col+r) {
      fx = 1;
    }
...
__global__ void kGetGlimpsesMatrixScanMR(float* F_x, float* F_y, int start_row, int start_col, int patch_side, int image_side, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int side_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (side_id < patch_side*image_side && case_id < num_cases) {
    F_x += case_id * patch_side * image_side + side_id;
    ...
    int fy = 0;
    if (side_id == r*image_side+start_row+r*2) {
      fy = 1;
    }

    int fx = 0;
    if (side_id == r*image_side+start_col+r*2) {
      fx = 1;
    }
...
__global__ void kGetGlimpsesMatrixScanMR3(float* F_x, float* F_y, int start_row, int start_col, int patch_side, int image_side, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int side_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (side_id < patch_side*image_side && case_id < num_cases) {
    F_x += case_id * patch_side * image_side + side_id;
    ...
    else if (side_id == r*image_side+start_row+r*2+1) {
      fy = 0.5;
    }
    ...
    else if (side_id == r*image_side+start_col+r*2+1) {
      fx = 0.5;
    }
...
__global__ void kGetGlimpsesMatrix(float* F_x, float* F_y, float* para, int patch_side, int image_side, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int side_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (side_id < patch_side*image_side && case_id < num_cases) {
    para += case_id * 5;
    ...
    int i = side_id / image_side;
    int a = side_id % image_side;
    float sigma = __expf(para[3]) * 1.0f * (image_side - 1) / (patch_side - 1);
    float mu_x = (para[0] + 1) * (image_side + 1) / 2.0f + (i - patch_side / 2.0f - 0.5) * sigma;
    float mu_y = (para[1] + 1) * (image_side + 1) / 2.0f + (i - patch_side / 2.0f - 0.5) * sigma;

    float fx = __expf(-(a - mu_x) * (a - mu_x) / (2.0f * __expf(para[2])));
    //fx = (fx < 1.0e-7) ? 1.0e-7 : fx;

    float fy = __expf(-(a - mu_y) * (a - mu_y) / (2.0f * __expf(para[2])));
    //fy = (fy < 1.0e-7) ? 1.0e-7 : fy;
...
__global__ void kGetGlimpsesMatrixLaplace(float* F_x, float* F_y, float* para, int patch_side, int image_side, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int side_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (side_id < patch_side*image_side && case_id < num_cases) {
    para += case_id * 5;
    ...
    float fx = 0.0f;
    if (a < mu_x) {
      fx = __expf((a - mu_x) / __expf(para[2]));
    }
    else {
      fx = __expf((mu_x - a) / __expf(para[2]));
    }
    //fx = (fx < 1.0e-7) ? 1.0e-7 : fx;

    float fy = 0.0f;
    if (a < mu_y) {
      fy = __expf((a - mu_y) / __expf(para[2]));
    }
    else {
      fy = __expf((mu_y - a) / __expf(para[2]));
    }
    //fy = (fy < 1.0e-7) ? 1.0e-7 : fy;
...
__global__ void kNormalizeGlmatrix(float* F_x, float* F_y, int patch_side, int image_side, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int patchside_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (patchside_id < patch_side && case_id < num_cases) {
    F_x += case_id * patch_side * image_side + patchside_id * image_side;
    F_y += case_id * patch_side * image_side + patchside_id * image_side;
...
__global__ void kApplyGlmatrixLeft(float* patches, float* F_axis, float* input_frame, int patch_side, int image_side, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int index_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (index_id < patch_side*image_side && case_id < num_cases) {
    patches += case_id * patch_side * image_side + index_id;
    F_axis += case_id * patch_side * image_side + (index_id / image_side) * image_side;
    input_frame += case_id * image_side * image_side + index_id % image_side;

    float sum = 0.0f;
    for (int i = 0; i < image_side; i++) {
      sum += F_axis[i]  * input_frame[i * image_side];
    }
...
__global__ void kApplyGlmatrixRight(float* patches, float* F_axis, float* para, float* input_frame, int patch_side, int image_side, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int index_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (index_id < patch_side*patch_side && case_id < num_cases) {
    patches += case_id * patch_side * patch_side + index_id;
    F_axis += case_id * patch_side * image_side + (index_id % patch_side) * image_side;
    para += case_id * 5;
    input_frame += case_id * patch_side * image_side + (index_id / patch_side) * image_side;

    float sum = 0.0f;
    for (int i = 0; i < image_side; i++) {
      sum += input_frame[i] * F_axis[i];
    }
...
__global__ void kApplyGlmatrixHatLeft(float* patches, float* F_axis, float* input_frame, int patch_side, int image_side, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int index_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (index_id < image_side*patch_side && case_id < num_cases) {
    patches += case_id * patch_side * image_side + index_id;
    F_axis += case_id * patch_side * image_side + index_id / patch_side;
    input_frame += case_id * patch_side * patch_side + index_id % patch_side;

    float sum = 0.0f;
    for (int i = 0; i < patch_side; i++) {
      sum += F_axis[i * image_side]  * input_frame[i * patch_side];
    }
...
__global__ void kApplyGlmatrixHatRight(float* patches, float* F_axis, float* para, float* input_frame, int patch_side, int image_side, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int index_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (index_id < image_side*image_side && case_id < num_cases) {
    patches += case_id * image_side * image_side + index_id;
    F_axis += case_id * patch_side * image_side + index_id % image_side;
    para += case_id * 5;
    input_frame += case_id * patch_side * image_side + (index_id / image_side) * patch_side;

    float sum = 0.0f;
    for (int i = 0; i < patch_side; i++) {
      sum += input_frame[i] * F_axis[i * image_side];
    }
...
__global__ void kGetSamplingGrid(float* grid_x, float* grid_y, float* para, int patch_side, int image_side, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int index_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (index_id < patch_side*patch_side && case_id < num_cases) {
    grid_x += case_id * patch_side * patch_side + index_id;
    grid_y += case_id * patch_side * patch_side + index_id;
    para += case_id * 6;

    int x = index_id / patch_side;
    int y = index_id % patch_side;
    float c = (patch_side - 1) / 2.0f;
    float c_im = (image_side - 1) / 2.0f;

    float x_n = (x - c) / c;
    float y_n = (y - c) / c;

    float x_s = para[0] * x_n + para[1] * y_n + para[2];
    float y_s = para[3] * x_n + para[4] * y_n + para[5];
...
__global__ void kGetAttenGrid(float* grid_x, float* grid_y, float* para, int patch_side, int image_side, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int index_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (index_id < patch_side*patch_side && case_id < num_cases) {
    grid_x += case_id * patch_side * patch_side + index_id;
    ...
    float x_s = para[0] * x_n + para[1];
    float y_s = para[2] * y_n + para[3];
    //float x_s = x_n + para[0];
    //float y_s = y_n + para[1];
...
__global__ void kGetAttenGridTrans(float* grid_x, float* grid_y, float* para, int patch_side, int image_side, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int index_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (index_id < patch_side*patch_side && case_id < num_cases) {
    grid_x += case_id * patch_side * patch_side + index_id;
...
__global__ void kApplySamplingGrid(float* patches, float* grid_x, float* grid_y, float* input_frame, int patch_side, int image_side, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int index_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (index_id < patch_side*patch_side && case_id < num_cases) {
    patches += case_id * patch_side * patch_side + index_id;
    grid_x += case_id * patch_side * patch_side + index_id;
    grid_y += case_id * patch_side * patch_side + index_id;
    input_frame += case_id * image_side * image_side;

    float g_x = grid_x[0];
    float g_y = grid_y[0];

    int image_x_tl = floor(g_x);
    int image_y_tl = floor(g_y);

    float w_x_tl = 1 - (g_x - image_x_tl);
    float w_y_tl = 1 - (g_y - image_y_tl);

    bool tl_in = between(image_x_tl, 0, image_side-1) && between(image_y_tl, 0, image_side-1);
    bool tr_in = between(image_x_tl, 0, image_side-1) && between(image_y_tl+1, 0, image_side-1);
    bool bl_in = between(image_x_tl+1, 0, image_side-1) && between(image_y_tl, 0, image_side-1);
    bool br_in = between(image_x_tl+1, 0, image_side-1) && between(image_y_tl+1, 0, image_side-1);
    ...
    if(tl_in) pixel_tl = input_frame[image_x_tl * image_side + image_y_tl];
    if(tr_in) pixel_tr = input_frame[image_x_tl * image_side + (image_y_tl+1)];
    if(bl_in) pixel_bl = input_frame[(image_x_tl+1) * image_side + image_y_tl];
    if(br_in) pixel_br = input_frame[(image_x_tl+1) * image_side + (image_y_tl+1)];

    sum = w_x_tl * w_y_tl * pixel_tl + w_x_tl * (1 - w_y_tl) * pixel_tr + (1 - w_x_tl) * w_y_tl * pixel_bl + (1 - w_x_tl) * (1 - w_y_tl) * pixel_br;
...
__global__ void kApplySamplingGridFeat(float* patches, float* grid_x, float* grid_y, float* input_map, int patch_side, int map_side, int num_channel, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int index_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (index_id < patch_side*patch_side && case_id < num_cases) {
    patches += case_id * num_channel * patch_side * patch_side + index_id;
    grid_x += case_id * patch_side * patch_side + index_id;
    grid_y += case_id * patch_side * patch_side + index_id;
    input_map += case_id * num_channel * map_side * map_side;

    float g_x = grid_x[0];
    float g_y = grid_y[0];

    int map_x_tl = floor(g_x);
    int map_y_tl = floor(g_y);

    float w_x_tl = 1 - (g_x - map_x_tl);
    float w_y_tl = 1 - (g_y - map_y_tl);

    bool tl_in = between(map_x_tl, 0, map_side-1) && between(map_y_tl, 0, map_side-1);
    bool tr_in = between(map_x_tl, 0, map_side-1) && between(map_y_tl+1, 0, map_side-1);
    bool bl_in = between(map_x_tl+1, 0, map_side-1) && between(map_y_tl, 0, map_side-1);
    bool br_in = between(map_x_tl+1, 0, map_side-1) && between(map_y_tl+1, 0, map_side-1);
    ...
    for (int k = 0; k < num_channel; k++) {
      ...
      if(tl_in) pixel_tl = input_map[k * map_side * map_side + map_x_tl * map_side + map_y_tl];
      if(tr_in) pixel_tr = input_map[k * map_side * map_side + map_x_tl * map_side + (map_y_tl+1)];
      if(bl_in) pixel_bl = input_map[k * map_side * map_side + (map_x_tl+1) * map_side + map_y_tl];
      if(br_in) pixel_br = input_map[k * map_side * map_side + (map_x_tl+1) * map_side + (map_y_tl+1)];

      sum = w_x_tl * w_y_tl * pixel_tl + w_x_tl * (1 - w_y_tl) * pixel_tr + (1 - w_x_tl) * w_y_tl * pixel_bl + (1 - w_x_tl) * (1 - w_y_tl) * pixel_br;

      __syncthreads();
      patches[k * patch_side * patch_side] = sum;
    }
  }
}

__global__ void kGetGridDeriveFeat(float* grid_x_deriv, float* grid_y_deriv, float* patch_deriv, float* grid_x, float* grid_y, float* input_map, int patch_side, int map_side, int num_channel, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int index_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (index_id < patch_side*patch_side && case_id < num_cases) {
    grid_x_deriv += case_id * patch_side * patch_side + index_id;
    grid_y_deriv += case_id * patch_side * patch_side + index_id;
    grid_x += case_id * patch_side * patch_side + index_id;
    grid_y += case_id * patch_side * patch_side + index_id;
    patch_deriv += case_id * num_channel * patch_side * patch_side + index_id;
    input_map += case_id * num_channel * map_side * map_side;
    ...
    for (int k = 0; k < num_channel; k++) {
      ...
      deriv_x += patch_deriv[k * patch_side * patch_side] * ((-1) * w_y_tl * pixel_tl + (-1) * (1 - w_y_tl) * pixel_tr + w_y_tl * pixel_bl + (1 - w_y_tl) * pixel_br);
      deriv_y += patch_deriv[k * patch_side * patch_side] * (w_x_tl * (-1) * pixel_tl + w_x_tl * pixel_tr + (1 - w_x_tl) * (-1) * pixel_bl + (1 - w_x_tl) * pixel_br);
    }

    __syncthreads();
    grid_x_deriv[0] = deriv_x;
    grid_y_deriv[0] = deriv_y;
  }
}

__global__ void kGetGridAndFeatDerive(float* grid_x_deriv, float* grid_y_deriv, float* feat_deriv, float* patch_deriv, float* grid_x, float* grid_y, float* input_map, int patch_side, int map_side, int num_channel, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int index_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (index_id < patch_side*patch_side && case_id < num_cases) {
    grid_x_deriv += case_id * patch_side * patch_side + index_id;
    grid_y_deriv += case_id * patch_side * patch_side + index_id;
    feat_deriv += case_id * num_channel * map_side * map_side * patch_side * patch_side + index_id;
    grid_x += case_id * patch_side * patch_side + index_id;
    grid_y += case_id * patch_side * patch_side + index_id;
    patch_deriv += case_id * num_channel * patch_side * patch_side + index_id;
    input_map += case_id * num_channel * map_side * map_side;
    ...
    for (int k = 0; k < num_channel; k++) {
      ...
      if(tl_in) feat_deriv[(k * map_side * map_side + map_x_tl * map_side + map_y_tl) * patch_side * patch_side] = patch_deriv[k * patch_side * patch_side] * w_x_tl *  w_y_tl;
      if(tr_in) feat_deriv[(k * map_side * map_side + map_x_tl * map_side + (map_y_tl+1)) * patch_side * patch_side] = patch_deriv[k * patch_side * patch_side] * w_x_tl * (1 - w_y_tl);
      if(bl_in) feat_deriv[(k * map_side * map_side + (map_x_tl+1) * map_side + map_y_tl) * patch_side * patch_side] = patch_deriv[k * patch_side * patch_side] * (1 - w_x_tl) *  w_y_tl;
      if(br_in) feat_deriv[(k * map_side * map_side + (map_x_tl+1) * map_side + (map_y_tl+1)) * patch_side * patch_side] = patch_deriv[k * patch_side * patch_side] * (1 - w_x_tl) *  (1 - w_y_tl);
    }
...
__global__ void kGetGridDerive(float* grid_x_deriv, float* grid_y_deriv, float* patch_deriv, float* grid_x, float* grid_y, float* input_frame, int patch_side, int image_side, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int index_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (index_id < patch_side*patch_side && case_id < num_cases) {
    grid_x_deriv += case_id * patch_side * patch_side + index_id;
    ...
    grid_x += case_id * patch_side * patch_side + index_id;
    grid_y += case_id * patch_side * patch_side + index_id;
    patch_deriv += case_id * patch_side * patch_side + index_id;
    input_frame += case_id * image_side * image_side;
    ...
    deriv_x = (-1) * w_y_tl * pixel_tl + (-1) * (1 - w_y_tl) * pixel_tr + w_y_tl * pixel_bl + (1 - w_y_tl) * pixel_br;
    deriv_y = w_x_tl * (-1) * pixel_tl + w_x_tl * pixel_tr + (1 - w_x_tl) * (-1) * pixel_bl + (1 - w_x_tl) * pixel_br;

    __syncthreads();
    grid_x_deriv[0] = patch_deriv[0] * deriv_x;
    grid_y_deriv[0] = patch_deriv[0] * deriv_y;
  }
}

__global__ void kGetParaAffineDerive(float* para_deriv, float* grid_x_deriv, float* grid_y_deriv, int patch_side, int image_side, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int index_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (index_id < patch_side*patch_side && case_id < num_cases) {
    grid_x_deriv += case_id * patch_side * patch_side + index_id;
    grid_y_deriv += case_id * patch_side * patch_side + index_id;
    para_deriv += case_id * 6;
    ...
    __syncthreads();
    para_deriv[0] = grid_x_deriv[0] * c_im * x_n;
    para_deriv[patch_side * patch_side] = grid_x_deriv[0] * c_im * y_n;
    para_deriv[2 * patch_side * patch_side] = grid_x_deriv[0] * c_im;
    para_deriv[3 * patch_side * patch_side] = grid_y_deriv[0] * c_im * x_n;
    para_deriv[4 * patch_side * patch_side] = grid_y_deriv[0] * c_im * y_n;
    para_deriv[5 * patch_side * patch_side] = grid_y_deriv[0] * c_im;
  }
}

__global__ void kGetParaAttenDerive(float* para_deriv, float* grid_x_deriv, float* grid_y_deriv, int patch_side, int image_side, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int index_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (index_id < patch_side*patch_side && case_id < num_cases) {
    grid_x_deriv += case_id * patch_side * patch_side + index_id;
    grid_y_deriv += case_id * patch_side * patch_side + index_id;
    para_deriv += case_id * 4 * patch_side * patch_side + index_id;
    ...
    __syncthreads();
    para_deriv[0] = grid_x_deriv[0] * c_im * x_n;
    para_deriv[patch_side * patch_side] = grid_x_deriv[0] * c_im;
    para_deriv[2 * patch_side * patch_side] = grid_y_deriv[0] * c_im * y_n;
    para_deriv[3 * patch_side * patch_side] = grid_y_deriv[0] * c_im;
  }
}

__global__ void kGetParaAttenTransDerive(float* para_deriv, float* grid_x_deriv, float* grid_y_deriv, int patch_side, int image_side, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int index_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (index_id < patch_side*patch_side && case_id < num_cases) {
    grid_x_deriv += case_id * patch_side * patch_side + index_id;
    grid_y_deriv += case_id * patch_side * patch_side + index_id;
    para_deriv += case_id * 2 * patch_side * patch_side + index_id;
    ...
    __syncthreads();
    para_deriv[0] = grid_x_deriv[0] * c;
    para_deriv[patch_side * patch_side] = grid_y_deriv[0] * c;
  }
}

__global__ void kSumParaDerive(float* para_deriv, float* para_grid_deriv, int num_para, int patch_side, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int index_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (index_id < num_para && case_id < num_cases) {
    para_grid_deriv += case_id * num_para * patch_side * patch_side + index_id * patch_side * patch_side;
    para_deriv += case_id * num_para + index_id;

    float sum = 0.0f;
    for (int k = 0; k < patch_side * patch_side; k++) {
      sum += para_grid_deriv[k];
    }
...
__global__ void kDisParaDerive(float* para_grid_deriv, float* para_deriv, int num_para, int patch_side, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int index_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (index_id < num_para && case_id < num_cases) {
    para_grid_deriv += case_id * num_para * patch_side * patch_side + index_id * patch_side * patch_side;
    ...
    float w_ave = patch_side * patch_side;
    for (int k = 0; k < patch_side * patch_side; k++) {
      __syncthreads();
      para_grid_deriv[k] += para_deriv[0] / w_ave;
    }
  }
}

__global__ void kGetPara0123Derive(float* para_xy_deriv, float* F_x, float* F_y, float* para, int patch_side, int image_side, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int index_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (index_id < patch_side*image_side && case_id < num_cases) {
    int i = index_id / image_side;
    int a = index_id % image_side;

    para_xy_deriv += case_id * patch_side * image_side * 6 + index_id;
    F_x += case_id * patch_side * image_side + i * image_side;
    F_y += case_id * patch_side * image_side + i * image_side;
    para += case_id * 5;
    ...
    float sum_x_013 = 0.0f;
    float sum_y_013 = 0.0f;
    float sum_x_2 = 0.0f;
    float sum_y_2 = 0.0f;
    for (int k = 0; k < image_side; k++) {
      sum_x_013 += F_x[k] * (k - mu_x) / __expf(para[2]);
      sum_y_013 += F_y[k] * (k - mu_y) / __expf(para[2]);
      sum_x_2 += F_x[k] * (k - mu_x) * (k - mu_x) / (2.0f * __expf(para[2]));
      sum_y_2 += F_y[k] * (k - mu_y) * (k - mu_y) / (2.0f * __expf(para[2]));
    }

    __syncthreads();
    para_xy_deriv[0] = (image_side - 1) / 2.0f * F_x[a] * ((a - mu_x) / __expf(para[2]) - sum_x_013);
    para_xy_deriv[patch_side * image_side] = (image_side - 1) / 2.0f * F_y[a] * ((a - mu_y) / __expf(para[2]) - sum_y_013);
    para_xy_deriv[2 * patch_side * image_side] = F_x[a] * ((a - mu_x) * (a - mu_x) / (2.0f * __expf(para[2])) - sum_x_2);
    para_xy_deriv[3 * patch_side * image_side] = F_y[a] * ((a - mu_y) * (a - mu_y) / (2.0f * __expf(para[2])) - sum_y_2);
    para_xy_deriv[4 * patch_side * image_side] = (i - patch_side / 2.0f - 0.5) * sigma * F_x[a] * ((a - mu_x) / __expf(para[2]) - sum_x_013);
    para_xy_deriv[5 * patch_side * image_side] = (i - patch_side / 2.0f - 0.5) * sigma * F_y[a] * ((a - mu_y) / __expf(para[2]) - sum_y_013);
  }
}

__global__ void kGetPara0123DeriveLaplace(float* para_xy_deriv, float* F_x, float* F_y, float* para, int patch_side, int image_side, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int index_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (index_id < patch_side*image_side && case_id < num_cases) {
    int i = index_id / image_side;
    ...
    float sum_y_2 = 0.0f;
    for (int k = 0; k < image_side; k++) {
      sum_x_013 += F_x[k] * 1.0f * ((k >= mu_x) - (k < mu_x)) / __expf(para[2]);
      sum_y_013 += F_y[k] * 1.0f * ((k >= mu_y) - (k < mu_y)) / __expf(para[2]);
      sum_x_2 += F_x[k] * 1.0f * ((k >= mu_x) - (k < mu_x)) * (k - mu_x) / __expf(para[2]);
      sum_y_2 += F_y[k] * 1.0f * ((k >= mu_y) - (k < mu_y)) * (k - mu_y) / __expf(para[2]);
    }

    __syncthreads();
    para_xy_deriv[0] = (image_side - 1) / 2.0f * F_x[a] * (1.0f * ((a >= mu_x) - (a < mu_x)) / __expf(para[2]) - sum_x_013);
    para_xy_deriv[patch_side * image_side] = (image_side - 1) / 2.0f * F_y[a] * (1.0f * ((a >= mu_y) - (a < mu_y)) / __expf(para[2]) - sum_y_013);
    para_xy_deriv[2 * patch_side * image_side] = F_x[a] * (1.0f * ((a >= mu_x) - (a < mu_x)) * (a - mu_x) / __expf(para[2]) - sum_x_2);
    para_xy_deriv[3 * patch_side * image_side] = F_y[a] * (1.0f * ((a >= mu_y) - (a < mu_y)) * (a - mu_y) / __expf(para[2]) - sum_y_2);
    para_xy_deriv[4 * patch_side * image_side] = (i - patch_side / 2.0f - 0.5) * sigma * F_x[a] * (1.0f * ((a >= mu_x) - (a < mu_x)) / __expf(para[2]) - sum_x_013);
    para_xy_deriv[5 * patch_side * image_side] = (i - patch_side / 2.0f - 0.5) * sigma * F_y[a] * (1.0f * ((a >= mu_y) - (a < mu_y)) / __expf(para[2]) - sum_y_013);
  }
}

__global__ void kGetOutputPatchDerive(float* output_patch_deriv, float* output_deriv, float* F_x_hat, float* F_y_hat, float* para_hat, int patch_side, int image_side, int num_cases) {
  const unsigned int case_id = blockIdx.y;
  const unsigned int index_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (index_id < patch_side*patch_side && case_id < num_cases) {
    int i = index_id / patch_side;
    int j = index_id % patch_side;

    output_patch_deriv += case_id * patch_side * patch_side + index_id;
    output_deriv += case_id * image_side * image_side;
    F_y_hat += case_id * patch_side * image_side + i * image_side;
    F_x_hat += case_id * patch_side * image_side + j * image_side;
    para_hat += case_id * 5;

    float sum = 0.0f;
    for (int m = 0; m < image_side; m++) {
      for (int n = 0; n < image_side; n++) {
        sum += F_y_hat[m] *  F_x_hat[n] * output_deriv[m * image_side + n];
      }
    }
    ...
    if (para_hat[4] == 0) {
      ...
      gama = __expf(para_hat[4]);
    }
    //gama = (gama < 1.0e-7) ? 1.0e-7 : gama;

    __syncthreads();
    output_patch_deriv[0] = sum / gama;
  }
}