...
def generate_exception(err_code):
    ...
    if err_code == -1:
        ...
        return CUDAMatException("Before call cublas error")
    elif err_code == -11:
        return CUDAMatException("Before call cuda error")
    else:
        return CUDAMatException("Error code %s" % err_code.__str__())
...
class CUDAMatrix(object):
    ...
    @staticmethod
    def GetOnesSlice(num_ones):
        if num_ones > CUDAMatrix.MAX_ONES:
            # This is not safe because it frees any previously held ones slice.
            # If this message is seen increase MAX_ONES.
            print 'Needed more ones : ', num_ones
            CUDAMatrix.MAX_ONES = num_ones
            CUDAMatrix.ones.free_device_memory()
            CUDAMatrix.ones = CUDAMatrix(np.ones((MAX_ONES, 1), dtype=np.float32, order = 'F'))
        return CUDAMatrix.ones.slice(0, num_ones)
    ...
    def sum(self, axis=None, target = None, mult=1.0):
        ...
        if axis is None:
          err_code = ct.c_int(0)
          res = _cudamat.sum_all(self.p_mat, CUDAMatrix.TEMP.p_mat, ct.byref(err_code))
          if err_code:
              raise generate_exception(err_code)
          return res
          """
          num_ones = self.shape[0] * self.shape[1]
          ones = CUDAMatrix.GetOnesSlice(num_ones)
          return vdot(self, ones) * mult
          """
        else:
    ...
    def add_sums(self, mat, axis, mult = 1.):
        ...
        if axis == 0:
            ...
            left = CUDAMatrix.GetOnesSlice(m)
            left.set_trans(True)
            right = mat
            ...
            left = mat
            right = CUDAMatrix.GetOnesSlice(n)
...
def empty(shape):
    ...
    if len(shape) == 2:
      ...
      err_code = _cudamat.init_empty(ct.pointer(mat), ct.c_int(shape[0]), ct.c_int(1))
    else:
...
def lstm_fprop2_init(gates, cell, output, w):
  num_lstms, num_cases = cell.shape
  assert gates.shape == (4 * num_lstms, num_cases)
  assert output.shape == cell.shape
  assert w.shape == (num_lstms, 3)
...
def lstm_bprop2(gates, gates_deriv, cell_prev, cell_prev_deriv, cell, cell_deriv, output_deriv, w):
  ...
  assert cell.shape  == (num_lstms, num_cases)
  assert cell_prev.shape == cell.shape
...
def lstm_outp2_init(gates_deriv, cell_state, dw):
  num_lstms, num_cases = cell_state.shape
  assert gates_deriv.shape == (4 * num_lstms, num_cases)
  assert cell_state.shape == (num_lstms, num_cases)
  assert dw.shape == (num_lstms, 3)
  err_code = _cudamat.lstm_outp2_init(gates_deriv.p_mat,
                                      cell_state.p_mat, dw.p_mat)
  if err_code:
    raise generate_exception(err_code)

def lstm_outp2(gates_deriv, prev_cell_state, cell_state, dw):
  num_lstms, num_cases = cell_state.shape
  assert gates_deriv.shape == (4 * num_lstms, num_cases)
  assert cell_state.shape == (num_lstms, num_cases)
  assert prev_cell_state.shape == cell_state.shape
  assert dw.shape == (num_lstms, 3)
  err_code = _cudamat.lstm_outp2(gates_deriv.p_mat, prev_cell_state.p_mat,
                                 cell_state.p_mat, dw.p_mat)
  if err_code:
    raise generate_exception(err_code)

def lstm_fprop_spatial_init(gates, cell, output, w):
  num_lstms, num_cases = cell.shape
  assert gates.shape == (5 * num_lstms, num_cases)
  assert output.shape == cell.shape
  assert w.shape == (num_lstms, 5)

  err_code = _cudamat.lstm_fprop_spatial_init(gates.p_mat, cell.p_mat, output.p_mat, w.p_mat)
  if err_code:
    raise generate_exception(err_code)

def lstm_fprop_spatial_row_init(gates, cell_prev_row, cell, output, w):
  num_lstms, num_cases = cell.shape
  assert gates.shape == (5 * num_lstms, num_cases)
  assert cell_prev_row.shape == cell.shape
  assert output.shape == cell.shape
  assert w.shape == (num_lstms, 5)

  err_code = _cudamat.lstm_fprop_spatial_row_init(gates.p_mat, cell_prev_row.p_mat, cell.p_mat, output.p_mat, w.p_mat)
  if err_code:
    raise generate_exception(err_code)

def lstm_fprop_spatial_col_init(gates, cell_prev_col, cell, output, w):
  num_lstms, num_cases = cell.shape
  assert gates.shape == (5 * num_lstms, num_cases)
  assert cell_prev_col.shape == cell.shape
  assert output.shape == cell.shape
  assert w.shape == (num_lstms, 5)

  err_code = _cudamat.lstm_fprop_spatial_row_init(gates.p_mat, cell_prev_col.p_mat, cell.p_mat, output.p_mat, w.p_mat)
  if err_code:
    raise generate_exception(err_code)

def lstm_fprop_spatial(gates, cell_prev_row, cell_prev_col, cell, output, w):
  num_lstms, num_cases = cell.shape
  assert gates.shape[1] == num_cases
  assert gates.shape[0] == 5 * num_lstms
  assert cell_prev_row.shape == cell.shape
  assert cell_prev_col.shape == cell.shape
  assert output.shape == cell.shape
  assert w.shape == (num_lstms, 5)

  err_code = _cudamat.lstm_fprop_spatial(gates.p_mat, cell_prev_row.p_mat, cell_prev_col.p_mat, cell.p_mat, output.p_mat, w.p_mat)
  if err_code:
    raise generate_exception(err_code)

def lstm_bprop_spatial_init(gates, gates_deriv, cell, cell_deriv, output_deriv, w):
  num_lstms, num_cases = cell.shape
  assert gates.shape == (5 * num_lstms, num_cases)
  assert cell.shape  == (num_lstms, num_cases)
  assert gates_deriv.shape == gates.shape
  ...
  err_code = _cudamat.lstm_bprop_spatial_init(gates.p_mat, gates_deriv.p_mat,
                                       cell.p_mat, cell_deriv.p_mat, output_deriv.p_mat, w.p_mat)
  if err_code:
    raise generate_exception(err_code)

def lstm_bprop_spatial_row_init(gates, gates_deriv, cell_prev_row, cell_prev_row_deriv, cell, cell_deriv, output_deriv, w):
  num_lstms, num_cases = cell.shape
  assert gates.shape == (5 * num_lstms, num_cases)
  assert cell.shape  == (num_lstms, num_cases)
  assert cell_prev_row.shape == cell.shape
  assert cell_prev_row_deriv.shape == cell.shape
  assert gates_deriv.shape == gates.shape
  assert cell_deriv.shape == cell.shape
  assert output_deriv.shape == cell.shape
  assert w.shape == (num_lstms, 5)

  err_code = _cudamat.lstm_bprop_spatial_row_init(gates.p_mat, gates_deriv.p_mat, cell_prev_row.p_mat, cell_prev_row_deriv.p_mat,
                                       cell.p_mat, cell_deriv.p_mat, output_deriv.p_mat, w.p_mat)
  if err_code:
    raise generate_exception(err_code)

def lstm_bprop_spatial_col_init(gates, gates_deriv, cell_prev_col, cell_prev_col_deriv, cell, cell_deriv, output_deriv, w):
  num_lstms, num_cases = cell.shape
  assert gates.shape == (5 * num_lstms, num_cases)
  assert cell.shape  == (num_lstms, num_cases)
  assert cell_prev_col.shape == cell.shape
  assert cell_prev_col_deriv.shape == cell.shape
  assert gates_deriv.shape == gates.shape
  assert cell_deriv.shape == cell.shape
  assert output_deriv.shape == cell.shape
  assert w.shape == (num_lstms, 5)

  err_code = _cudamat.lstm_bprop_spatial_col_init(gates.p_mat, gates_deriv.p_mat, cell_prev_col.p_mat, cell_prev_col_deriv.p_mat,
                                       cell.p_mat, cell_deriv.p_mat, output_deriv.p_mat, w.p_mat)
  if err_code:
    raise generate_exception(err_code)

def lstm_bprop_spatial(gates, gates_deriv, cell_prev_row, cell_prev_row_deriv, cell_prev_col, cell_prev_col_deriv, cell, cell_deriv, output_deriv, w):
  num_lstms, num_cases = cell.shape
  assert gates.shape == (5 * num_lstms, num_cases)
  assert gates_deriv.shape == gates.shape
  assert cell.shape  == (num_lstms, num_cases)
  ...
  err_code = _cudamat.lstm_bprop_spatial(gates.p_mat, gates_deriv.p_mat, cell_prev_row.p_mat, cell_prev_row_deriv.p_mat, cell_prev_col.p_mat,
                                  cell_prev_col_deriv.p_mat, cell.p_mat, cell_deriv.p_mat, output_deriv.p_mat, w.p_mat)
  if err_code:
    raise generate_exception(err_code)

def lstm_outp_spatial_init(gates_deriv, cell_state, dw):
  num_lstms, num_cases = cell_state.shape
  assert gates_deriv.shape == (5 * num_lstms, num_cases)
  assert cell_state.shape == (num_lstms, num_cases)
  assert dw.shape == (num_lstms, 5)
  err_code = _cudamat.lstm_outp_spatial_init(gates_deriv.p_mat,
                                      cell_state.p_mat, dw.p_mat)
  if err_code:
    raise generate_exception(err_code)

def lstm_outp_spatial_row_init(gates_deriv, prev_cell_row_state, cell_state, dw):
  num_lstms, num_cases = cell_state.shape
  assert gates_deriv.shape == (5 * num_lstms, num_cases)
  assert cell_state.shape == (num_lstms, num_cases)
  assert prev_cell_row_state.shape == cell_state.shape
  assert dw.shape == (num_lstms, 5)
  err_code = _cudamat.lstm_outp_spatial_row_init(gates_deriv.p_mat, prev_cell_row_state.p_mat,
                                      cell_state.p_mat, dw.p_mat)
  if err_code:
    raise generate_exception(err_code)

def lstm_outp_spatial_col_init(gates_deriv, prev_cell_col_state, cell_state, dw):
  num_lstms, num_cases = cell_state.shape
  assert gates_deriv.shape == (5 * num_lstms, num_cases)
  assert cell_state.shape == (num_lstms, num_cases)
  assert prev_cell_col_state.shape == cell_state.shape
  assert dw.shape == (num_lstms, 5)
  err_code = _cudamat.lstm_outp_spatial_col_init(gates_deriv.p_mat, prev_cell_col_state.p_mat,
                                      cell_state.p_mat, dw.p_mat)
  if err_code:
    raise generate_exception(err_code)

def lstm_outp_spatial(gates_deriv, prev_cell_row_state, prev_cell_col_state, cell_state, dw):
  num_lstms, num_cases = cell_state.shape
  assert gates_deriv.shape == (5 * num_lstms, num_cases)
  assert cell_state.shape == (num_lstms, num_cases)
  assert prev_cell_row_state.shape == cell_state.shape
  assert prev_cell_col_state.shape == cell_state.shape
  assert dw.shape == (num_lstms, 5)
  err_code = _cudamat.lstm_outp_spatial(gates_deriv.p_mat, prev_cell_row_state.p_mat, prev_cell_col_state.p_mat,
                                 cell_state.p_mat, dw.p_mat)
  if err_code:
    raise generate_exception(err_code)

def get_glimpses_matrix_scan(F_x, F_y, start_row, start_col, patch_side, image_side):
  num_side, num_cases = F_x.shape
  assert num_side == patch_side * image_side
  assert F_y.shape == (num_side, num_cases)
  assert F_x.shape == (num_side, num_cases)

  err_code = _cudamat.get_glimpses_matrix_scan(F_x.p_mat, F_y.p_mat, ct.c_int(start_row), ct.c_int(start_col),
                                          ct.c_int(patch_side), ct.c_int(image_side), ct.c_int(num_cases))
  if err_code:
    raise generate_exception(err_code)

def get_glimpses_matrix_scan_mr(F_x, F_y, start_row, start_col, patch_side, image_side):
  num_side, num_cases = F_x.shape
  assert num_side == patch_side * image_side
  assert F_y.shape == (num_side, num_cases)
  assert F_x.shape == (num_side, num_cases)

  err_code = _cudamat.get_glimpses_matrix_scan_mr(F_x.p_mat, F_y.p_mat, ct.c_int(start_row), ct.c_int(start_col),
                                          ct.c_int(patch_side), ct.c_int(image_side), ct.c_int(num_cases))
  if err_code:
    raise generate_exception(err_code)

def get_glimpses_matrix_scan_mr3(F_x, F_y, start_row, start_col, patch_side, image_side):
  num_side, num_cases = F_x.shape
  assert num_side == patch_side * image_side
  assert F_y.shape == (num_side, num_cases)
  assert F_x.shape == (num_side, num_cases)

  err_code = _cudamat.get_glimpses_matrix_scan_mr3(F_x.p_mat, F_y.p_mat, ct.c_int(start_row), ct.c_int(start_col),
                                          ct.c_int(patch_side), ct.c_int(image_side), ct.c_int(num_cases))
  if err_code:
    raise generate_exception(err_code)

def get_glimpses_matrix(F_x, F_y, para, patch_side, image_side):
  num_side, num_cases = F_x.shape
  assert num_side == patch_side * image_side
  assert F_y.shape == (num_side, num_cases)
  assert F_x.shape == (num_side, num_cases)

  err_code = _cudamat.get_glimpses_matrix(F_x.p_mat, F_y.p_mat, para.p_mat,
                                          ct.c_int(patch_side), ct.c_int(image_side))
  if err_code:
    raise generate_exception(err_code)

def get_glimpses_matrix_laplace(F_x, F_y, para, patch_side, image_side):
  num_side, num_cases = F_x.shape
  assert num_side == patch_side * image_side
  assert F_y.shape == (num_side, num_cases)
  assert F_x.shape == (num_side, num_cases)

  err_code = _cudamat.get_glimpses_matrix_laplace(F_x.p_mat, F_y.p_mat, para.p_mat,
                                          ct.c_int(patch_side), ct.c_int(image_side))
  if err_code:
    raise generate_exception(err_code)

def normalize_glmatrix(F_x, F_y, patch_side, image_side):
  err_code = _cudamat.normalize_glmatrix(F_x.p_mat, F_y.p_mat,
                                         ct.c_int(patch_side), ct.c_int(image_side))
  if err_code:
    raise generate_exception(err_code)

def apply_glmatrix(patches, F_axis, para, input_frame, patch_side, image_side):
  err_code = _cudamat.apply_glmatrix(patches.p_mat, F_axis.p_mat, para.p_mat, input_frame.p_mat,
                                         ct.c_int(patch_side), ct.c_int(image_side))
  if err_code:
    raise generate_exception(err_code)

def apply_glmatrix_hat(patches, F_axis, para, input_frame, patch_side, image_side):
  err_code = _cudamat.apply_glmatrix_hat(patches.p_mat, F_axis.p_mat, para.p_mat, input_frame.p_mat,
                                         ct.c_int(patch_side), ct.c_int(image_side))
  if err_code:
    raise generate_exception(err_code)

def get_sampling_grid(grid_x, grid_y, para, patch_side, image_side):
  err_code = _cudamat.get_sampling_grid(grid_x.p_mat, grid_y.p_mat, para.p_mat,
                                         ct.c_int(patch_side), ct.c_int(image_side))
  if err_code:
    raise generate_exception(err_code)

def get_atten_grid(grid_x, grid_y, para, patch_side, image_side):
  err_code = _cudamat.get_atten_grid(grid_x.p_mat, grid_y.p_mat, para.p_mat,
                                        ct.c_int(patch_side), ct.c_int(image_side))
  if err_code:
    raise generate_exception(err_code)

def get_atten_grid_trans(grid_x, grid_y, para, patch_side, image_side):
  err_code = _cudamat.get_atten_grid_trans(grid_x.p_mat, grid_y.p_mat, para.p_mat,
                                           ct.c_int(patch_side), ct.c_int(image_side))
  if err_code:
    raise generate_exception(err_code)

def apply_sampling_grid(patches, grid_x, grid_y, input_frame, patch_side, image_side):
  err_code = _cudamat.apply_sampling_grid(patches.p_mat, grid_x.p_mat, grid_y.p_mat, input_frame.p_mat,
                                         ct.c_int(patch_side), ct.c_int(image_side))
  if err_code:
    raise generate_exception(err_code)

def apply_sampling_grid_feat(patches, grid_x, grid_y, input_map, patch_side, map_side):
  err_code = _cudamat.apply_sampling_grid_feat(patches.p_mat, grid_x.p_mat, grid_y.p_mat, input_map.p_mat,
                                         ct.c_int(patch_side), ct.c_int(map_side))
  if err_code:
    raise generate_exception(err_code)

def get_grid_derive_feat(grid_x_deriv, grid_y_deriv, patch_deriv, grid_x, grid_y, input_map, patch_side, map_side):
  err_code = _cudamat.get_grid_derive_feat(grid_x_deriv.p_mat, grid_y_deriv.p_mat, patch_deriv.p_mat, grid_x.p_mat, grid_y.p_mat, input_map.p_mat,
                                         ct.c_int(patch_side), ct.c_int(map_side))
  if err_code:
    raise generate_exception(err_code)

def get_grid_and_feat_derive(grid_x_deriv, grid_y_deriv, feat_deriv, patch_deriv, grid_x, grid_y, input_map, patch_side, map_side):
  err_code = _cudamat.get_grid_and_feat_derive(grid_x_deriv.p_mat, grid_y_deriv.p_mat, feat_deriv.p_mat, patch_deriv.p_mat, grid_x.p_mat, grid_y.p_mat, input_map.p_mat,
                                         ct.c_int(patch_side), ct.c_int(map_side))
  if err_code:
    raise generate_exception(err_code)

def get_grid_derive(grid_x_deriv, grid_y_deriv, patch_deriv, grid_x, grid_y, input_frame, patch_side, image_side):
  err_code = _cudamat.get_grid_derive(grid_x_deriv.p_mat, grid_y_deriv.p_mat, patch_deriv.p_mat, grid_x.p_mat, grid_y.p_mat, input_frame.p_mat,
                                         ct.c_int(patch_side), ct.c_int(image_side))
  if err_code:
    raise generate_exception(err_code)

def get_para_affine_derive(para_affine_deriv, grid_x_deriv, grid_y_deriv, patch_side, image_side):
  err_code = _cudamat.get_para_affine_derive(para_affine_deriv.p_mat, grid_x_deriv.p_mat, grid_y_deriv.p_mat,
                                         ct.c_int(patch_side), ct.c_int(image_side))
  if err_code:
    raise generate_exception(err_code)

def get_para_atten_derive(para_affine_deriv, grid_x_deriv, grid_y_deriv, patch_side, image_side):
  err_code = _cudamat.get_para_atten_derive(para_affine_deriv.p_mat, grid_x_deriv.p_mat, grid_y_deriv.p_mat,
                                         ct.c_int(patch_side), ct.c_int(image_side))
  if err_code:
    raise generate_exception(err_code)

def get_para_atten_trans_derive(para_affine_deriv, grid_x_deriv, grid_y_deriv, patch_side, image_side):
  err_code = _cudamat.get_para_atten_trans_derive(para_affine_deriv.p_mat, grid_x_deriv.p_mat, grid_y_deriv.p_mat,
                                           ct.c_int(patch_side), ct.c_int(image_side))
  if err_code:
    raise generate_exception(err_code)

def sum_para_derive(para_deriv, para_grid_deriv, patch_side):
  err_code = _cudamat.sum_para_derive(para_deriv.p_mat, para_grid_deriv.p_mat, ct.c_int(patch_side))
  if err_code:
    raise generate_exception(err_code)

def dis_para_derive(para_grid_deriv, para_deriv, patch_side):
  err_code = _cudamat.dis_para_derive(para_grid_deriv.p_mat, para_deriv.p_mat, ct.c_int(patch_side))
  if err_code:
    raise generate_exception(err_code)

def get_para0123_derive(para_xy_deriv, F_x, F_y, para_atten, patch_side, image_side):
  err_code = _cudamat.get_para0123_derive(para_xy_deriv.p_mat, F_x.p_mat, F_y.p_mat, para_atten.p_mat,
                                         ct.c_int(patch_side), ct.c_int(image_side))
  if err_code:
    raise generate_exception(err_code)

def get_para0123_derive_laplace(para_xy_deriv, F_x, F_y, para_atten, patch_side, image_side):
  err_code = _cudamat.get_para0123_derive_laplace(para_xy_deriv.p_mat, F_x.p_mat, F_y.p_mat, para_atten.p_mat,
                                         ct.c_int(patch_side), ct.c_int(image_side))
  if err_code:
    raise generate_exception(err_code)

def get_output_patch_derive(output_patch_deriv, output_deriv, F_x_hat, F_y_hat, para_atten_hat, patch_side, image_side):
  err_code = _cudamat.get_output_patch_derive(output_patch_deriv.p_mat, output_deriv.p_mat, F_x_hat.p_mat, F_y_hat.p_mat, para_atten_hat.p_mat,
                                         ct.c_int(patch_side), ct.c_int(image_side))
  if err_code:
    raise generate_exception(err_code)
...
def cuda_set_p2p(dev_id1, dev_id2):
    """
    Selects the CUDA device with the given ID.
    """

    err_code =  _cudamat.cuda_set_P2P(ct.c_int(dev_id1), ct.c_int(dev_id2))
    if err_code:
        raise generate_exception(err_code)
...
def cublas_init():
    ...
    CUDAMatrix.ones = CUDAMatrix(np.ones((CUDAMatrix.MAX_ONES, 1), dtype=np.float32, order = 'F'))
    CUDAMatrix.TEMP = empty((8, 1))
...
def cublas_shutdown():
    ...
    """
    CUDAMatrix.ones.free_device_memory()
    CUDAMatrix.TEMP.free_device_memory()
